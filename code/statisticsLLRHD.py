...
output_file = os.path.join(directory_path, sorted_subdirectories[0], 'count_array.txt')
...
for directory in sorted_subdirectories[1:]:
    ...
    deleted_count, kept_count, count_array= read_and_statistics_data(input_file, threshold, count_array) #调用主函数
...
output_png = os.path.join(directory_path, sorted_subdirectories[0], 'threshold_'+str(threshold)+'.png')
# 保存图表为图片
